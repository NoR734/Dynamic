...
function ISCharacterKills:render()
    ...
    if md and self.javaObject then
        ...
        if not selectedPlayer or selectedPlayer <= 1 then
            ...
            if (not killCountModData or not killCountModData.WeaponCategory) and SandboxVars.KillCount.shareOnServer then
                local playerName = lcl.player_getUsername(self.char)
                local gmd = KCShared.getModData()
                if gmd and playerName and gmd[playerName] then
                    killCountModData = gmd[playerName]
                    md.KillCount = killCountModData
                    if killCountModData.AKCModData then
                        md.AKCModData = killCountModData.AKCModData
                    end
                end
            end
        else
