...
function KCServer.savePlayerModData(player, data)
    if not player or not data then return end

    local pmd = player:getModData()
    if pmd then
        pmd.KillCount = data
        if data.AKCModData then
            pmd.AKCModData = data.AKCModData
        end
    end
end
...
function KCServer.OnClientCommand(mod, command, player, args)
    ...
    if command == player:getUsername() then
        ...
        local gmd = ModData.getOrCreate(KCShared.Key)
        ...
        KCServer.savePlayerModData(player, args)
        
        if SandboxVars.KillCount.shareOnServer then
            ModData.transmit(KCShared.Key);--share with all subscribed clients --TODO limit server emission frequency
        end
    end
end
